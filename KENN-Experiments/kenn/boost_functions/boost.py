...
class BoostFunction(torch.nn.Module, abc.ABC):
    ...
    def forward(self, selected_predicates: torch.Tensor, signs: torch.Tensor, edge_weight: torch.Tensor):
        """
...
class GodelBoostConormApprox(BoostFunction):
    ...
    def forward(self, selected_predicates: torch.Tensor, signs: torch.Tensor, edge_weight: torch.Tensor):
        ...
        edge_weight = edge_weight.repeat(1, clause_matrix.shape[1])
        return signs * softmax(clause_matrix, dim=-1) * self.clause_weight * edge_weight
...
class GodelBoostConorm(BoostFunction):
    ...
    def forward(self, selected_predicates: torch.Tensor, signs: torch.Tensor, edge_weight: torch.Tensor):
        ...
        edge_weight = torch.transpose(edge_weight.repeat(clause_matrix.shape[1], 1), 0, 1)
        delta[np.arange(delta.size()[0]), indices] = self.clause_weight * signs[indices] * edge_weight
        # Approximated Godel t-conorm boost function on preactivations
        return delta
...
class LukasiewiczBoostConorm(BoostFunction):
    ...
    def forward(self, selected_predicates: torch.Tensor, signs: torch.Tensor, edge_weight: torch.Tensor):
        ...
        return torch.ones(clause_matrix.shape) * ((sums < 1) *
                                                  (1 - sums) *
                                                  self.clause_weight / clause_matrix.shape[1])[:,
                                                 None] * signs * edge_weight
...
class ProductBoostConorm(BoostFunction):
    ...
    def forward(self, selected_predicates: torch.Tensor, signs: torch.Tensor, edge_weight: torch.Tensor):
        ...
        return preactivation_delta * signs * edge_weight
